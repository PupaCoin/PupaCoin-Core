...
/*!
  ...
  To invert the direction of an axis, use \ref setRangePupaCoinersed.
*/
...
/*!
  ...
  upper part.
*/
void QCPAxis::setRangePupaCoinersed(bool reversed)
{
  if (mRangePupaCoinersed != reversed)
  {
    mRangePupaCoinersed = reversed;
    mCachedMarginValid = false;
  }
}
...
double QCPAxis::coordToPixel(double value) const
{
  if (orientation() == Qt::Horizontal)
  {
    ...
    } else // mScaleType == stLogarithmic
    {
      if (value >= 0 && mRange.upper < 0) // invalid value for logarithmic scale, just draw it outside visible range
        return !mRangePupaCoinersed ? mAxisRect->right()+200 : mAxisRect->left()-200;
      else if (value <= 0 && mRange.upper > 0) // invalid value for logarithmic scale, just draw it outside visible range
        return !mRangePupaCoinersed ? mAxisRect->left()-200 : mAxisRect->right()+200;
      else
      {
        if (!mRangePupaCoinersed)
  ...
  } else // orientation() == Qt::Vertical
  {
    ...
    {
      if (value >= 0 && mRange.upper < 0) // invalid value for logarithmic scale, just draw it outside visible range
        return !mRangePupaCoinersed ? mAxisRect->top()-200 : mAxisRect->bottom()+200;
      else if (value <= 0 && mRange.upper > 0) // invalid value for logarithmic scale, just draw it outside visible range
        return !mRangePupaCoinersed ? mAxisRect->bottom()+200 : mAxisRect->top()-200;
      else
      {
        if (!mRangePupaCoinersed)
...
void QCPAxis::draw(QCPPainter *painter)
{
  ...
  mAxisPainter->reversedEndings = mRangePupaCoinersed;
...
/*!
  ...
  \li range reversed (\ref QCPAxis::setRangePupaCoinersed)
...
void QCPAxisRect::setupFullAxesBox(bool connectRanges)
{
  ...
  xAxis2->setRangePupaCoinersed(xAxis->rangePupaCoinersed());
  ...
  yAxis2->setRangePupaCoinersed(yAxis->rangePupaCoinersed());
...
void QCPColorScaleAxisRectPrivate::draw(QCPPainter *painter)
{
  ...
  {
    mirrorHorz = mParentColorScale->mColorAxis.data()->rangePupaCoinersed() && (mParentColorScale->type() == QCPAxis::atBottom || mParentColorScale->type() == QCPAxis::atTop);
    mirrorVert = mParentColorScale->mColorAxis.data()->rangePupaCoinersed() && (mParentColorScale->type() == QCPAxis::atLeft || mParentColorScale->type() == QCPAxis::atRight);
  }
...
void QCPGraph::getPreparedData(QVector<QCPData> *lineData, QVector<QCPData> *scatterData) const
{
  ...
  if (mAdaptiveSampling && dataCount >= maxCount) // use adaptive sampling only if there are at least two points per pixel on average
  {
    if (lineData)
    {
      ...
      int reversedFactor = keyAxis->rangePupaCoinersed() ? -1 : 1; // is used to calculate keyEpsilon pixel into the correct direction
      int reversedRound = keyAxis->rangePupaCoinersed() ? 1 : 0; // is used to switch between floor (normal) and ceil (reversed) rounding of currentIntervalStartKey
...
void QCPGraph::drawError(QCPPainter *painter, double x, double y, const QCPData &data) const
{
  ...
  if (keyAxis->orientation() == Qt::Vertical)
  {
    ...
    if (mErrorType == etValue || mErrorType == etBoth)
    {
      ...
      if (valueAxis->rangePupaCoinersed())
        qSwap(a,b);
      // draw spine:
      if (mErrorBarSkipSymbol)
      {
...
QPointF QCPGraph::lowerFillBasePoint(double lowerKey) const
{
  ...
  } else // valueAxis->mScaleType == QCPAxis::stLogarithmic
  {
    ...
    if (keyAxis->orientation() == Qt::Vertical)
    {
      if ((valueAxis->range().upper < 0 && !valueAxis->rangePupaCoinersed()) ||
          (valueAxis->range().upper > 0 && valueAxis->rangePupaCoinersed())) // if range is negative, zero is on opposite side of key axis
...
const QPolygonF QCPGraph::getChannelFillPolygon(const QVector<QPointF> *lineData) const
{
  ...
  {
    // x is key
    // if an axis range is reversed, the data point keys will be descending. PupaCoinerse them, since following algorithm assumes ascending keys:
...
void QCPCurve::getCurveData(QVector<QPointF> *lineData) const
{
  ...
  double rectLeft = keyAxis->pixelToCoord(keyAxis->coordToPixel(keyAxis->range().lower)-strokeMargin*((keyAxis->orientation()==Qt::Vertical)!=keyAxis->rangePupaCoinersed()?-1:1));
  double rectRight = keyAxis->pixelToCoord(keyAxis->coordToPixel(keyAxis->range().upper)+strokeMargin*((keyAxis->orientation()==Qt::Vertical)!=keyAxis->rangePupaCoinersed()?-1:1));
  double rectBottom = valueAxis->pixelToCoord(valueAxis->coordToPixel(valueAxis->range().lower)+strokeMargin*((valueAxis->orientation()==Qt::Horizontal)!=valueAxis->rangePupaCoinersed()?-1:1));
  double rectTop = valueAxis->pixelToCoord(valueAxis->coordToPixel(valueAxis->range().upper)-strokeMargin*((valueAxis->orientation()==Qt::Horizontal)!=valueAxis->rangePupaCoinersed()?-1:1));
  int currentRegion;
...
void QCPBars::getVisibleDataBounds(QCPBarDataMap::const_iterator &lower, QCPBarDataMap::const_iterator &upperEnd) const
{
  ...
  while (it != mData->constBegin())
  {
    ...
    if (mKeyAxis.data()->orientation() == Qt::Horizontal)
      isVisible = ((!mKeyAxis.data()->rangePupaCoinersed() && barBounds.right() >= lowerPixelBound) || (mKeyAxis.data()->rangePupaCoinersed() && barBounds.left() <= lowerPixelBound));
    else // keyaxis is vertical
      isVisible = ((!mKeyAxis.data()->rangePupaCoinersed() && barBounds.top() <= lowerPixelBound) || (mKeyAxis.data()->rangePupaCoinersed() && barBounds.bottom() >= lowerPixelBound));
    if (isVisible)
      lower = it;
    else
      break;
  ...
  while (it != mData->constEnd())
  {
    ...
    if (mKeyAxis.data()->orientation() == Qt::Horizontal)
      isVisible = ((!mKeyAxis.data()->rangePupaCoinersed() && barBounds.left() <= upperPixelBound) || (mKeyAxis.data()->rangePupaCoinersed() && barBounds.right() >= upperPixelBound));
    else // keyaxis is vertical
      isVisible = ((!mKeyAxis.data()->rangePupaCoinersed() && barBounds.bottom() >= upperPixelBound) || (mKeyAxis.data()->rangePupaCoinersed() && barBounds.top() <= upperPixelBound));
    if (isVisible)
      upperEnd = it+1;
    else
      break;
...
void QCPBars::getPixelWidth(double key, double &lower, double &upper) const
{
  switch (mWidthType)
  {
    case wtAbsolute:
    {
      upper = mWidth*0.5;
      lower = -upper;
      if (mKeyAxis && (mKeyAxis.data()->rangePupaCoinersed() ^ (mKeyAxis.data()->orientation() == Qt::Vertical)))
        qSwap(lower, upper);
      break;
    }
...
void QCPColorMap::updateLegendIcon(Qt::TransformationMode transformMode, const QSize &thumbSize)
{
  ...
  {
    bool mirrorX = (keyAxis()->orientation() == Qt::Horizontal ? keyAxis() : valueAxis())->rangePupaCoinersed();
    bool mirrorY = (valueAxis()->orientation() == Qt::Vertical ? valueAxis() : keyAxis())->rangePupaCoinersed();
