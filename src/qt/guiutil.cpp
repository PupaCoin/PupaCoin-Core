...
{
    widget->setMaxLength(PupaCoinAddressValidator::MaxAddressLength);
    widget->setValidator(new PupaCoinAddressValidator(parent));
...
bool parsePupaCoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    // NovaCoin: check prefix
    if(uri.scheme() != QString("PupaCoin"))
        return false;
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if(!PupaCoinUnits::parse(PupaCoinUnits::PPCN, i->second, &rv.amount))
                {
                    return false;
                }
            }
...
bool parsePupaCoinURI(QString uri, SendCoinsRecipient *out)
{
    // Convert rev:// to PupaCoin:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("rev://", Qt::CaseInsensitive))
    {
        uri.replace(0, 11, "PupaCoin:");
    }
    QUrl uriInstance(uri);
    return parsePupaCoinURI(uriInstance, out);
}
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "PupaCoin.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for PupaCoin.lnk
...
{
    return GetAutostartDir() / "PupaCoin.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        // Write a PupaCoin.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=PupaCoin\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the PupaCoin app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if(fAutoStart && !foundItem) {
        // add PupaCoin app to startup item list
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("PupaCoin-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  PupaCoin-qt [" + tr("command-line options") + "]                     " + "\n";
