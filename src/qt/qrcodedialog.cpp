...
{
    QString ret = QString("PupaCoin:%1").arg(address);
    int paramCount = 0;
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non PPCN unit input in lnReqAmount, we generate the URI with PPCN as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(PupaCoinUnits::format(PupaCoinUnits::PPCN, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
