...
namespace {
    class CPupaCoinAddressVisitor : public boost::static_visitor<bool> {
    private:
        CPupaCoinAddress *addr;
    public:
        CPupaCoinAddressVisitor(CPupaCoinAddress *addrIn) : addr(addrIn) { }
...
bool CPupaCoinAddress::Set(const CKeyID &id) {
    SetData(Params().Base58Prefix(CChainParams::PUBKEY_ADDRESS), &id, 20);
    return true;
}

bool CPupaCoinAddress::Set(const CScriptID &id) {
    SetData(Params().Base58Prefix(CChainParams::SCRIPT_ADDRESS), &id, 20);
    return true;
}

bool CPupaCoinAddress::Set(const CTxDestination &dest) {
    return boost::apply_visitor(CPupaCoinAddressVisitor(this), dest);
}

bool CPupaCoinAddress::IsValid() const {
...
CTxDestination CPupaCoinAddress::Get() const {
    if (!IsValid())
...
bool CPupaCoinAddress::GetKeyID(CKeyID &keyID) const {
...
bool CPupaCoinAddress::IsScript() const {
    return IsValid() && vchVersion == Params().Base58Prefix(CChainParams::SCRIPT_ADDRESS);
}

void CPupaCoinSecret::SetKey(const CKey& vchSecret) {
...
CKey CPupaCoinSecret::GetKey() {
    CKey ret;
    ret.Set(&vchData[0], &vchData[32], vchData.size() > 32 && vchData[32] == 1);
    return ret;
}

bool CPupaCoinSecret::IsValid() const {
    bool fExpectedFormat = vchData.size() == 32 || (vchData.size() == 33 && vchData[32] == 1);
    bool fCorrectVersion = vchVersion == Params().Base58Prefix(CChainParams::SECRET_KEY);
    return fExpectedFormat && fCorrectVersion;
}

bool CPupaCoinSecret::SetString(const char* pszSecret) {
    return CBase58Data::SetString(pszSecret) && IsValid();
}

bool CPupaCoinSecret::SetString(const std::string& strSecret) {
